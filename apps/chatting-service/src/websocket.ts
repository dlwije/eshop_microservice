...
export async function createWebSocketServer(server: HttpServer) {
  ...
  wss.on("connection", (ws: WebSocket) => {
    ...
    ws.on("message", async (rawMessage) => {
      try {
        ...
        // Registerthe user on first plain message (non-JSON)
        if (!registeredUserId && !message.startsWith("{")) {
        ...
        if (receiverSocket && receiverSocket.readyState === WebSocket.OPEN) {
          ...
          console.log(`Deliverd message + unseen count to ${receiverKey}`);
        } else {
          console.log(`User ${receiverKey} is offline. Message queued.`);
        }
        // send message(echo) to sender
        ...
        console.log(`message queueed to kafka: ${conversationId}`);
      } catch (error) {
    ...
    ws.on("close", async () => {
      if (registeredUserId) {
        ...
        console.log(`User ${registeredUserId} removed from online`);
      }
    });
  ...
  console.log("WebSocket server ready started!");
}
