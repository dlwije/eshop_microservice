  ...
  const fetchFilteredProducts = async () => {
    ...
    try {
      ...
      const res = await axiosInstance.get(
        `/product/api/v1/get-filtered-offers?${query.toString()}`,
      );
      ...
      <div className="w-[90%] lg:w-[80%] m-auto">
        ...
        <div className="w-full flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1 px-2 lg:px-3">
            {isProductLoading ? (
              <div className="gris grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-4">
                {Array.from({ length: 10 }).map((_, index) => (
                  <div
                    key={index}
                    className="h-[250px] bg-gray-300 animate-pulse rounded-lg"
                  ></div>
                ))}
              </div>
            ) : products.length > 0 ? (
